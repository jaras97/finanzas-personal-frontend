...
- `src/app/(app)/` — shell autenticado: `summary`, `transactions`, `recurring`, `saving-accounts`, `debts`, `categories`, `budgets`, `admin`. Tiene su propio `layout.tsx` que renderiza `Sidebar` + `Footer` y hace **gating de suscripción del lado del cliente**.
...
- Rutas privadas (por prefijo): `/summary`, `/transactions`, `/recurring`, `/saving-accounts`, `/categories`, `/budgets`, `/debts`, `/admin` → sin token o token inválido, redirige a `/auth/login`. El middleware solo verifica que haya **sesión válida**; que además sea admin lo valida el backend (403) y la propia página `/admin` al montar.
